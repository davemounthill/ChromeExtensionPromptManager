...
function nextPromptTemplatesPage() {
    ...
    promptTemplateSection.currentPage++
        promptTemplateSection.currentPage = Math.min(Math.floor(
            (check.length - 1) / promptTemplateSection.pageSize), promptTemplateSection.currentPage)
    // Update the section
    updateTemplates(globalTemplates, "", "", globalTags)
}
...
function CSVToArray(strData, strDelimiter) {
    ...
    var pattern = new RegExp("(\\" + strDelimiter + "|\\r?\\n|\\r|^)" + "(?:\"([^\"]*(?:\"\"[^\"]*)*)\"|" + "([^\"\\" + strDelimiter + "\\r\\n]*))", "gi");
    var data = [
        []
    ];
    var matchesf;
